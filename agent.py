...
from replay_buffer import NStepPrioritizedExperienceReplay, NStepReplayBuffer, ReplayBuffer
...
class DQN:
    ...
    def calculate_loss(self, batch_index):
        '''
        Depends on whether we are using NStepPrioritizedExperienceReplay or not
        '''
        ...
        if per:
            tderror = T.abs(q_pred - q_target)
            self.replay_buffer.update_priority(tderror, index)
            loss = tderror.pow(2).mean()
            return T.mean(loss*weights)
        else:
